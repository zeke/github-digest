...
export async function sendEmail(config: CloudflareEmailConfig, email: DigestEmail): Promise<void> {
	...
	if (!data.success) {
		throw new Error(`Cloudflare Email Sending failed: ${JSON.stringify(data.errors)}`);
	}
}
