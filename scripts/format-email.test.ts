...
import { buildDigestEmail, formatPacificDate, markdownToHtml } from './format-email.ts';

describe('markdownToHtml', () => {
	it('renders headings, bullet lists, and links', () => {
		const markdown = ['## Needs your action', '- [Fix bug](https://github.com/zeke/x/pull/1) in `zeke/x`'].join('\n');
	...
	it('renders plain lines as paragraphs', () => {
		expect(markdownToHtml('Nothing new today.')).toBe('<p>Nothing new today.</p>');
	});

	it('escapes HTML special characters', () => {
		expect(markdownToHtml('A <script> tag & friends')).toBe('<p>A &lt;script&gt; tag &amp; friends</p>');
	});

	it('closes a list when a blank line or new section follows', () => {
		const markdown = ['- one', '- two', '', '## Next'].join('\n');
		expect(markdownToHtml(markdown)).toBe(['<ul>', '<li>one</li>', '<li>two</li>', '</ul>', '<h2>Next</h2>'].join('\n'));
	});
});
...
describe('formatPacificDate', () => {
	it('formats a date in the America/Los_Angeles timezone', () => {
		...
		expect(formatPacificDate(new Date('2026-08-19T15:00:00Z'))).toBe('Aug 19, 2026');
	});
});

describe('buildDigestEmail', () => {
	it('builds the full email payload', () => {
		const email = buildDigestEmail('Nothing new today.', new Date('2026-08-19T15:00:00Z'));
